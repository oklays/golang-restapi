...
func GetPostgresDB() (*sql.DB, error) {
	...
	databaseSchema := os.Getenv("POSTGRES_DB_SCHEMA")

	desc := fmt.Sprintf("host=%s user=%s password=%s dbname=%s sslmode=disable search_path=%s", host, user, password, databaseName, databaseSchema)
